...
// PURPOSE: The AI post creation page at /dashboard/create
//          User enters an idea + tone + goal.
//          Claude generates platform-native posts for LinkedIn, Instagram, X, Reddit.
//          User can edit each post, regenerate a single platform,
//          publish now, schedule (syncs to Google Calendar), or save as draft.
...
> = {
  linkedin:  { label: "LinkedIn",  icon: Linkedin,      color: "#0A66C2", hint: "Professional tone, storytelling, 150-300 words" },
  instagram: { label: "Instagram", icon: Instagram,     color: "#E1306C", hint: "Casual, emojis, hashtags, 80-150 words" },
  x:         { label: "X",         icon: Radio,         color: "#ffffff", hint: "Punchy, max 280 chars" },
  reddit:    { label: "Reddit",    icon: MessageCircle, color: "#FF4500", hint: "Authentic, no hashtags, community-focused" },
};
    ...
    <div className="mx-auto max-w-4xl px-6 py-10">
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Enter one idea — AI generates unique posts for every platform.
        </p>
      </div>
      ...
      {generating && (
        <div className="mt-8 grid gap-3 md:grid-cols-2">
          {PLATFORMS.map((p) => (
            <div key={p} className="h-48 rounded-2xl border border-white/8 bg-white/[0.02] animate-pulse" />
          ))}
        </div>
      )}
      ...
      {posts && !generating && (
        <div className="mt-8">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="text-xs text-zinc-600">Click a tab · edit directly · then publish or schedule</span>
          </div>
              ...
              <div key={platform} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <div className="mt-1 flex items-center justify-between">
                  <span className={`text-xs ${
                    platform === "x" && posts[platform].length > 280
                      ? "text-red-400 font-semibold"
                      : "text-zinc-700"
                  }`}>
                    {posts[platform].length} chars
                    {platform === "x" && posts[platform].length > 280 && " — over limit!"}
                  </span>
                </div>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-white/8 pt-4">
                  <button
                    ...
                    disabled={!!publishing || (platform === "x" && posts[platform].length > 280)}
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 rounded-xl border border-white/10 px-4 py-2 text-sm font-medium text-zinc-600 hover:text-zinc-400 transition-colors"
                  >
        ...
        >
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-zinc-950 p-6 shadow-2xl">
            ...
            <p className="text-xs text-zinc-500 mb-4">
              Scheduling to {PLATFORM_META[scheduleModal].label}.
