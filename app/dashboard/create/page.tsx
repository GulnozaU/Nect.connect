...
const PLATFORM_META: Record<Platform, {
  ...
  linkedin: { label: "LinkedIn", icon: Linkedin, color: "#0A66C2", hint: "Short lines + blank lines (feed-native) · hashtags at end" },
...
export default function CreatePostPage() {
  ...
  async function generatePosts() {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          idea: idea.trim(), tone, goal, language,
  ...
  async function regeneratePlatform(platform: Platform) {
    ...
    try {
      const res = await fetch("/api/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ idea: idea.trim(), tone, goal, language, platforms: [platform], customInstructions: userInstructions }),
      });
      const data = await res.json();
  ...
  function openPublishConfirm(platform: Platform) {
    if (!posts?.[platform]) return;
    setPublishConfirm({ mode: "now", platform });
  }

  async function executeConfirmedPublish() {
    if (!publishConfirm || !posts?.[publishConfirm.platform]) return;
    const { mode, platform, scheduledAt } = publishConfirm;
    setPublishing(platform);
    setScheduling(mode === "schedule");
    try {
      if (mode === "now") {
        ...
        const res = await fetch("/api/publish", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ text: posts[platform], platform, scheduledAt }),
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error ?? "Schedule failed");
        showToast(
          "success",
          `Scheduled for ${new Date(scheduledAt).toLocaleString(undefined, { weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}`
        );
      }
      setPublishConfirm(null);
    } catch (err) {
      showToast("error", err instanceof Error ? err.message : mode === "schedule" ? "Schedule failed" : "Publish failed");
    } finally {
      setPublishing(null);
      setScheduling(false);
    }
  ...
  function requestScheduleSecondConfirm() {
    if (!posts || !scheduleModal || !scheduleDate || !scheduleTime) return;
    const scheduledAt = new Date(`${scheduleDate}T${scheduleTime}`).toISOString();
    const scheduleSummary = new Date(scheduledAt).toLocaleString(undefined, {
      weekday: "short",
      month: "short",
      day: "numeric",
      hour: "numeric",
    ...
    <div className="mx-auto max-w-5xl px-5 py-10 md:px-8">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5 sm:p-6">
        ...
        <div className="flex flex-wrap items-end gap-3 border-t border-white/8 pt-5">
          ...
          <div className="min-w-[130px] flex-1">
            <label className="mb-1.5 block text-[10px] text-zinc-600">Language</label>
            <div className="relative">
            ...
            return (
              <div key={platform} className={`rounded-2xl border bg-white/[0.02] p-5 transition-all ${
                ...
                <div className="flex flex-wrap gap-2 border-t border-white/8 pt-4">
                  <button onClick={() => openPublishConfirm(platform)}
          ...
          {generatedPlatforms.length > 1 && (
            <div className="mt-4 flex justify-end">
              <p className="text-xs text-zinc-600">
                Use <span className="text-zinc-400">Post now</span> on each tab and confirm — one safety check per platform.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 px-4"
          ...
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-zinc-950 p-6 shadow-2xl">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={requestScheduleSecondConfirm} disabled={!scheduleDate || !scheduleTime}
      ...
      {publishConfirm && posts && (
        <PublishConfirmModal
          open
          mode={publishConfirm.mode === "schedule" ? "schedule" : "publish_now"}
          platformLabel={PLATFORM_META[publishConfirm.platform].label}
          postText={posts[publishConfirm.platform] ?? ""}
          scheduleSummary={publishConfirm.scheduleSummary ?? null}
          loading={Boolean(publishing || scheduling)}
          onCancel={() => {
            if (publishing || scheduling) return;
            setPublishConfirm(null);
          }}
          onConfirm={() => void executeConfirmedPublish()}
        />
      )}
    </div>
  );
